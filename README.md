...
### 🚀 2026
* **`02월`** 🩺 **MedGemma Sentinel** (Kaggle 의료 AI 에이전트)
* **`02월`** 🧮 **AIMO Prize 3 Solver** (Kaggle 수학 올림피아드 AI)
* **`02월`** 💔 **심장 질환 예측** (Kaggle Playground S6E2)
* **`01월`** 🌤️ **Weather Life MCP v3.7** (PlayMCP 공모전)
* **`01월`** ⚽ **K리그 패스 좌표 예측 AI 경진대회** (DACON 상위 7%)

---

### 🌟 2025
* **`12월`** 🧬 **유전체 변이 감지 AI 경진대회** (DACON 101위)
* **`11월`** 🎙️ **SilverLink - AI 복지 도우미** (경북대 AI-conic 해커톤 최우수상/2등)
* **`11월`** 🧩 **Kaggle ARC Prize 2025** (AGI 시각 추론 패턴 분석)
* **`10월`** 📈 **토스 광고 CTR 예측 AI 경진대회** (DACON)
* **`10월`** 📊 **Hull Tactical Market** (Kaggle 금융 시계열 예측)
* **`10월`** 🌊 **해양수산부 이상감지 AI 경진대회** (DACON 상위 236위)
* **`09월`** 🚗 **자동차 뉴스 분류 AI 경진대회** (DACON 자체 검증 100%)
* **`06월 - 08월`** 🍔 **YUM:AI - 아동 한식 이미지 영양 분석 서비스** (SKT Fly AI 최고 평가 수료)
...
- **주요 성과**:
  - MedGemma 1.5-4B 모델 기반의 **Dual-Agent Architecture** 설계. 의사 진료용 트리아지 에이전트(Agent A)와 환자용 요약 에이전트(Agent B) 역할 분담.
...
> **AGI 추론 벤치마크 달성을 위한 추상적 시각 패턴 인식 모델** (2025.11)
- **주요 성과**:
  - 추상적이고 복잡한 격자 패턴 데이터의 색상, 형태 변화 규칙을 인지하는 논리 추론 파이프라인 개발 및 실험 진행.
...
> **센서 데이터를 분석하여 해양 이상 상태를 감지하는 분류 모델** (2025.10)
- **주요 성과**:
  - 센서 신호 데이터를 활용하여 **상위 236위** 기록.
  - 149개의 물리 센서 피처 분석 및 이상 감지(Anomaly Detection)를 위한 파생 피처 생성 기법 적용.
